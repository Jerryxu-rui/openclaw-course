...
| **USER.md** | 关于用户的偏好、习惯、重要事实。Agent通过这里了解你 | 每次Session启动时 |
| **MEMORY.md** | 长期记忆存储，记录重要决策、事实和上下文 | 只在main/private session中加载 |
| **HEARTBEAT.md** | 定时任务配置，定义Agent在空闲时自动执行的任务 | 心跳触发时 |
| **memory/YYYY-MM-DD.md** | 每日交互日志，提供对话连续性上下文 | Session开始时读取今天和昨天的日志 |

### 工作区特点
1. **纯文本存储**：所有配置和记忆都是可读的Markdown文件
2. **版本控制友好**：适合用Git管理，方便备份和协作
3. **人类可编辑**：你可以直接修改这些文件，无需通过Agent
4. **持久化设计**：文件系统保证了记忆的持久性，不会因会话结束而丢失
5. **模块化结构**：每个文件有明确职责，便于维护和扩展
