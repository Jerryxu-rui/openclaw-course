...
ClawHub是OpenClaw的官方技能市场，包含13,729个注册技能，但需要谨慎选择。

#### 市场概况

| 指标 | 数据 | 说明 |
|------|------|------|
| 总注册技能 | 13,729 | 所有注册的技能数量 |
| 精选技能 | 5,494 | 经过awesome列表筛选的高质量技能 |
| 被过滤技能 | 6,940 | 垃圾、重复或低质量内容 |
| 被标记为恶意 | 800+ | 约20%在高峰期被标记 |

**重要提醒**：ClawHub的质量问题非常严重。社区项目`awesome-openclaw-skills`（31.4K Stars）从13,729个技能中只精选了5,494个，剩下的大部分是垃圾、重复或低质量内容。安装任何第三方Skill前，务必查看源码。
...
**特点**：ClawHub支持向量搜索，也就是说你可以用自然语言描述需求来搜索Skill，不必精确匹配名称。

#### 安全建议
1. **查看源码**：安装前务必查看技能源码，了解其功能
2. **使用精选列表**：优先使用`awesome-openclaw-skills`精选的技能
3. **限制权限**：为第三方技能设置适当的权限限制
4. **定期更新**：定期更新技能，获取安全修复
5. **监控使用**：监控技能的使用情况和资源消耗

### 2.4 热门Skills推荐

OpenClaw内置55个技能开箱即用，加上社区精选的必装Top 10。
...
| 10 | Image Gen | 中 | 图像生成和处理，支持多种AI绘图模型。 |

#### 选择建议
1. **新手必装**：Summarize、Web Search、Translator
2. **开发者必装**：GitHub、Claude Code、Agent Browser
3. **办公必装**：Gmail/Google、Calendar、File Manager
4. **创意必装**：Image Gen、视频处理相关技能
