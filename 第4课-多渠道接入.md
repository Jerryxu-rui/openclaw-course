...
| 飞书 | 官方API | 内置 | 简单 | 10-15分钟 |
| 钉钉 | 官方API | 内置 | 简单 | 15-20分钟 |
| 企业微信 | 官方API | 内置 | 中等 | 20-30分钟 |
| 微信 | 扩展 | 扩展 | 困难 | 45-60分钟 |
| QQ | 扩展 | 扩展 | 困难 | 45-60分钟 |
| 微博 | 扩展 | 扩展 | 中等 | 30-45分钟 |
| Twitch | 官方API | 扩展 | 中等 | 20-30分钟 |
| YouTube | 官方API | 扩展 | 中等 | 25-40分钟 |

### 渠道分类说明

#### 内置渠道（开箱即用）
- Telegram、Discord、WhatsApp、Slack、Signal、Google Chat
- 飞书、钉钉、企业微信
- 配置简单，文档完善

#### 扩展渠道（需要额外安装）
- iMessage、LINE、Microsoft Teams、Matrix、Mattermost
- IRC、Nostr、微信、QQ、微博、Twitch、YouTube
- 可能需要额外依赖或配置
...
### 选择建议
1. **新手推荐**：Telegram（最简单）、飞书（中文友好）
2. **团队协作**：Slack、Discord、Microsoft Teams
3. **中国市场**：飞书、钉钉、企业微信
4. **隐私优先**：Signal、Matrix、Nostr
5. **社交媒体**：微信、微博、YouTube
...
OpenClaw Gateway默认监听本地`ws://127.0.0.1:18789`。当你需要从外部网络访问时，有以下几种方案。
...
转发后，本地客户端连接`ws://127.0.0.1:18789`即可访问远程Gateway。
...
**安全提醒**：v2026.3.7起Gateway认证要求显式设置`gateway.auth.mode`（token或password）。不要在公网暴露未认证的Gateway。
...
**核心建议**：如果你同时使用多台设备，推荐Tailscale Serve + macOS菜单栏应用的组合：Mac运行Gateway和菜单栏应用，手机/平板通过Tailscale网络访问。
