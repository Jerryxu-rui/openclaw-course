...
#### Daily Logs日志系统
每天的交互记录以append-only方式写入`memory/YYYY-MM-DD.md`文件。Session开始时，Agent会自动读取今天和昨天的日志，为对话提供连续性上下文。
...
1. **检测阈值**：Session token用量接近上限，触发Pre-Compaction流程
2. **静默保存**：Agent在后台执行一个隐藏turn，将重要记忆写入MEMORY.md和Daily Log
...
**搜索工具**：
- `memory_search`：语义搜索，返回约400 token的chunks，适合回忆模糊的上下文
